 ...
 */
public class BuildTemporalPoint {
	static final Logger s_logger = LoggerFactory.getLogger(BuildTemporalPoint.class);
	...
	private long m_periodMillis = UnitUtils.parseDuration("30m").toMillis();
	...
	public void setPeriod(String period) {
		m_periodMillis = UnitUtils.parseDuration(period).toMillis();
	}
	...
	public void run(Dataset<Row> input) {
		...
		rows = buildTemporalPointWithinPartition(rows);
		rows = mergeTemporalPoints(rows);
