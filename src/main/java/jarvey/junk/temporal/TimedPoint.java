...
public class TimedPoint implements Row {
	...
	public TimedPoint(GeometryBean ptRow, long ts) {
		m_pt = ptRow;
		m_ts = ts;
	}
	
	public GeometryBean getGeometryRow() {
		return m_pt;
	}
	
	public Point getPoint() {
		return (Point)m_pt.asGeometry();
	}
	...
	public static TimedPoint fromRow(Row row) {
		if ( row instanceof TimedPoint ) {
			...
			GeometryBean ptRow = GeometryBean.fromWkb(row.getAs(0));
			long ts = row.getAs(1);
	...
	public String toString() {
		Point pt = (Point)m_pt.asGeometry();
