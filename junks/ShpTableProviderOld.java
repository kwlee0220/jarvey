 ...
 * @author Kang-Woo Lee
 */
public class ShpTableProviderOld implements TableProvider {
	...
	public StructType inferSchema(CaseInsensitiveStringMap options) {
		...
		try {
			...
			// Shapefile 파일에서 첫번째 레코드를 읽어 Geometry type을 얻는다.
			Geometry firstGeom = shpFile.streamGeometries().findFirst().get();
			Geometries geoms = Geometries.get(firstGeom);
			
			// SimpleFeatureType에서는 SRID 정보를 얻는다.
			int srid = getSrid(sfType);
			m_defGeomType = GeometryType.of(geoms, srid);
